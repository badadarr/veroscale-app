...
async function getReportConfigurationById(res: NextApiResponse, id: string) {
  try {
    ...
    if (reportConfig.recipients && typeof reportConfig.recipients === 'string') {
  ...
  try {
    ...
    if (!name || !type || !fields) {
      return res.status(400).json({ 
        message: 'Name, type, and fields are required for a report configuration' 
      });
    }
    ...
    if (useSupabase) {
      ...
      if (duplicateCheck && duplicateCheck.length > 0 && duplicateCheck[0].id !== id) {
        return res.status(400).json({ message: 'A report configuration with this name already exists' });
      }
    ...
    return res.status(200).json({ message: 'Report configuration updated successfully' });
  } catch (error) {
...
async function deleteReportConfiguration(res: NextApiResponse, id: string) {
  try {
    ...
    return res.status(200).json({ message: 'Report configuration deleted successfully' });
  } catch (error) {
