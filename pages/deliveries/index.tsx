...
export default function Deliveries() {
  ...
  useEffect(() => {
    if (user && !["admin", "marketing", "operator"].includes(user.role)) {
  ...
  const fetchDeliveries = async () => {
    try {
      const { data } = await apiClient.get(
        `/api/deliveries?page=${pagination.currentPage}&limit=${pagination.itemsPerPage}`
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.post("/api/deliveries", {
        ...
        expected_weight: formData.expected_weight
          ? parseFloat(formData.expected_weight)
          : null,
      });
                    ...
                    <div>
                      ...
                      >
                        ...
                        {samples.map((sample) => (
                          <option
                            key={sample.id}
                            value={`${sample.category} - ${sample.item}`}
                          >
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          expected_weight: e.target.value,
                        })
                      }
                    />
                    <Input
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  if (canShipToday(delivery.scheduled_date)) {
                                    ...
                                    toast.error(
                                      "Cannot ship before scheduled date"
                                    );
                                  }
                                }}
                              >
                ...
                <div className="mt-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="text-sm text-gray-600">
                      ...
                      of {pagination.totalItems} deliveries
                    </div>
                  </div>
